...
data_path = 'DRG_selection_HOM.xlsx';
...
cell_ids = T_mat(2:198,1);
SPNPs = T_mat(2:198,2);
C_nt = T_mat(1:197,3:333);
...
filename = 'Norm_dat_SEM_HOM.xlsx';
...
data_path = 'DRG_selection_WT.xlsx';
Twt = readtable(data_path);
T_mat_wt = table2array(Twt); 

C_nt_wt = T_mat_wt(1:179,3:end);


mean_data_wt = mean(C_nt_wt)
time_window1_wt = mean_data_wt(:, 90:150)
time_window2_wt = mean_data_wt(:, 180:240)
time_window3_wt = mean_data_wt(:, 270:330)

time_window1_norm_wt = time_window1_wt - min(time_window1_wt);
time_window2_norm_wt = time_window2_wt- min(time_window2_wt);
time_window3_norm_wt = time_window3_wt - min(time_window3_wt);

font_sz = 40;
lw = 4;
figure;
plot(x1,time_window1_norm_wt)
xlabel('Time (s)');
ylabel('Ratio 340:380');
title('Decay dynamics 1st peak - normalised to 0');
set(gca,'FontSize',font_sz);
...
set(gca,'FontSize',font_sz);
set(gcf,'color','w');
legend('SEM','Mean');  %% this code didn't work --> std_act = std(C_nt); std_error = std_act/sqrt(N);shadedErrorBar(t_s, mean_act, std_error);
...
%% Plot the data
cell_choose = 62; %Choose the cell [1-529]
peak = 2; %Choose the peak [1-3]
...
filename = 'Tau_values_hom_DRG.xlsx';
