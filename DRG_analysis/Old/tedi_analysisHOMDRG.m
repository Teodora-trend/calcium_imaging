...
data_path = 'DRG hom cells.xlsx';
T = readtable(data_path);
T_mat = table2array(T); 
t_s = T_mat(3:end,1); %Take all the time points
C_tn = T_mat(3:end,2:end); %Take all the cells and time points and store in a matrix
...
title('Mean activity across Neurons - HOM');
...
xlabel('Neuron #');
ylabel('Neuron #');
title('Correlation coef between all pairs of neurons - HOM');
set(gca,'FontSize',font_sz);
set(gcf,'color','w');
colormap('redblue');
colorbar;
...
xlabel('Time (s)');
ylabel('Neuron #');
title('Normalized Activity of all neurons sorted by reliability (SPNP) - HOM');
set(gca,'FontSize',font_sz);
set(gcf,'color','w');
colormap('viridis');
colorbar;
...
set(gca,'XTickLabel',[]);
ylabel('Neuron #');
title(['Normalized Activity of all selected HOM neurons with SPNP Th>', num2str(SPSP_th)]);
set(gca,'FontSize',font_sz);
set(gcf,'color','w');
colormap('viridis');
colorbar;
